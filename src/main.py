...
def assign_stage_to_model_version(model_name, version, stage):
    ...
    try:
        ...
        print(f"✅ Version {version} du modèle '{model_name}' assignée au stage '{stage}'.")
...
def main(train_path=None, test_path=None, prepare_only_flag=False, predict_flag=False, mlflow_flag=False, register_flag=False, stage=None, model_version=None, assign_stage=None):
    ...
    if assign_stage:
        if not model_version:
            raise ValueError("❌ Vous devez spécifier une version avec --model_version pour assigner un stage.")
    ...
    if predict_flag:
        ...
        if stage and model_version:
            raise ValueError("❌ Vous ne pouvez pas spécifier à la fois un stage et une version. Utilisez l'un ou l'autre.")
        elif stage:
            ...
            raise ValueError("❌ Vous devez spécifier soit un stage, soit une version pour charger le modèle.")
        ...
        sample_features = np.random.rand(1, 20)  # Ajustez les dimensions en fonction des données d'entraînement (20 colonnes)
